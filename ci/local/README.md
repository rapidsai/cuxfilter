...
Example Usage:
`bash build.sh -r ~/rapids/cuxfilter -i gpuci/rapidsai:0.16-cuda10.2-devel-ubuntu16.04-py3.7`
...
The docker image will generate build artifacts in a folder on your machine located in the `root` directory of the repository you passed to the script. For the above example, the directory is named `~/rapids/cuxfilter/build_rapidsai_cuda10.1-ubuntu16.04-py3.7/`. Feel free to remove this directory after the script is finished.
