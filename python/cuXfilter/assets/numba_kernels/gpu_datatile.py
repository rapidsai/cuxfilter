...
def calc_data_tile_for_size(
    df, col_1, min_1, max_1, stride_1,
    cumsum: bool = True, return_format='pandas'
):
    ...
    groupby_result = df[[col_1 + '_mod']].groupby(
        col_1 + '_mod', method='hash', as_index=True
    ...
    groupby_result_array = (
        groupby_result[groupby_result.columns[-1]].to_array())
...
def calc_data_tile(
    df, active_view: Type[BaseChart], passive_view: Type[BaseChart],
    aggregate_fn: str = '', cumsum: bool = True, return_format='pandas'
):
    '''
    description:
    ...
    col_1, min_1, max_1, stride_1 = (
        active_view.x, active_view.min_value,
        active_view.max_value, active_view.stride
    )
    col_2, min_2, max_2, stride_2 = (
        passive_view.x, passive_view.min_value,
        passive_view.max_value, passive_view.stride
    )
    ...
    if(key == col_1 and col_1 + '_mod' not in df.columns):
        df.add_column(
            col_1 + '_mod', get_binwise_reduced_column(
                df[col_1].copy().to_gpu_array(), stride_1, a1_range
            )
        )
    ...
    if(key == col_2 and col_2 + '_mod' not in df.columns):
        df.add_column(
            col_2 + '_mod', get_binwise_reduced_column(
