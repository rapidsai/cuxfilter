...
class TestDataFrame():
    ...
    #integrated tests
    @pytest.mark.parametrize('active_view, passive_view, result',[
        ('key_line', 'val_bar', pd.DataFrame(
    ...
    def test_calc_data_tiles(self, active_view, passive_view, result):
        ...
        assert dashboard._charts[dashboard._active_view].datatile_loaded_state == True
    ...
    def test_query_datatiles_by_indices(self, old_indices, new_indices, prev_result, result):
        df = cudf.DataFrame(
            {'key': [0, 1, 2, 3, 4], 'val':[float(i + 10) for i in range(5)]}
            )
        cux_df = cuXfilter.DataFrame.from_dataframe(df)
        ...
        dashboard._query_datatiles_by_indices(old_indices=old_indices, new_indices=new_indices)
    ...
    def test_reset_current_view(self):
        ...
        assert dashboard._charts[bac.name].datatile_loaded_state == False
        assert bac1.name not in dashboard._query_str_dict
        assert dashboard._data.equals(df.query(dashboard._query_str_dict['key_line']))