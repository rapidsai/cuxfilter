...
class TestViewDataFrame():
    ...
    def test_initiate_chart(self):
        vd = ViewDataFrame()
        assert vd.columns == None

        vd.initiate_chart(self.dashboard)

        assert str(vd.chart.objects[0]) == str(pn.pane.HTML(self.df, 
                                style={'width':'100%', 'height':'100%',
                                 'overflow-y':'auto', 'font-size':'0.5vw',
                                 'overflow-x':'auto'}))
    ...
    @pytest.mark.parametrize('width, height, result1, result2', [
        (400, 400, 400, 400),
        (None, None, 400, 400)
    ])
    ...
    def test_query_chart_by_range(self):
        ...
        bsl.query_chart_by_range(active_chart = bsl_1, query_tuple=query_tuple, data=df)
    ...
    def test_query_chart_by_indices(self, new_indices, result):
        ...
        #creating a dummy reload chart fn as its not implemented in core non aggregate chart class
        bsl.reload_chart = t_func
        bsl.query_chart_by_indices(active_chart = bsl_1, old_indices=[], new_indices=new_indices, data=df)
