...
import cuXfilter
from cuXfilter.charts.core.aggregate.core_datasize_indicator import BaseDataSizeIndicator
...
class TestBaseDataSizeIndicator():
    ...
    def test_variables(self):
        ...
        assert bdsi.use_data_tiles == True
    ...
    def test_view(self, chart, _chart):
        ...
        assert str(bdsi.view()) == str(chart_view(_chart,  css_classes=['non-handle-temp']))
    ...
    @pytest.mark.parametrize( 'old_indices, new_indices, prev_value, result', [
        ([], [1], 5.0, 1.0),
        ([1], [2], 1.0, 1.0),
        ([2], [2,4], 1.0, 2.0),
    ])
    def test_query_chart_by_indices(self, old_indices, new_indices, prev_value, result):
        active_chart = BaseBar(x='key')
        active_chart.stride = 1
        active_chart.min_value = 0
        self.result = ''
        ...
        bdsi.query_chart_by_indices(active_chart, old_indices, new_indices, datatile)
