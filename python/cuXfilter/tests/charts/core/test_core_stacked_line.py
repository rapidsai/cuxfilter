...
from cuXfilter.charts.core.non_aggregate.core_stacked_line import BaseStackedLine
...
class TestBaseStackedLine():
    ...
    def test_variables(self):
        ...
        assert bsl.add_interaction == True
        assert bsl._stride == None
        assert bsl.stride_type == int
        assert bsl.height == 400
        assert bsl.width == 800
        assert bsl._library_specific_params == {}
        assert bsl.chart == None
        assert bsl.source == None
        assert bsl.source_backup == None
        assert bsl.min_value == 0.0
        assert bsl.max_value == 0.0
        assert bsl.name == 'key_stacked_lines'

        #BaseStackedLineChart variables
        assert bsl.use_data_tiles == False
        assert bsl.reset_event == None
        assert bsl.x_range == None
        assert bsl.y_range == None
        assert bsl.colors == []

    def test_exceptions(self):
        with pytest.raises(TypeError):
            bsl = BaseStackedLine(x='key', y='val')
            bsl = BaseStackedLine(x='key', y=['val'], colors='color1')
        with pytest.raises(ValueError):
            bsl = BaseStackedLine(x='key', y=[])
    ...
    def test_get_selection_geometry_callback(self):
        ...
        assert bsl.get_selection_geometry_callback(self.dashboard).__name__ == 'selection_callback'
        assert type(bsl.get_selection_geometry_callback(self.dashboard)) == types.FunctionType
    ...
    def test_selection_callback(self):
        ...
        assert self.dashboard._query_str_dict['key_stacked_lines'] == '1<=key <= 2'
    
    @pytest.mark.parametrize('x_range, y_range, query',[((1,2), (3,4), '1<=a <= 2'), ((0,2), (3,5), '0<=a <= 2')])
    ...
    @pytest.mark.parametrize('add_interaction, reset_event, event_1, event_2',
                            [(True, None, 'selection_callback', None), (True, 'test_event', 'selection_callback', 'reset_callback'), (False, 'test_event', None, 'reset_callback')])
    ...
    def test_query_chart_by_range(self):
        ...
        #creating a dummy reload chart fn as its not implemented in core non aggregate chart class
        bsl.reload_chart = t_func
        bsl.query_chart_by_range(active_chart = bsl_1, query_tuple=query_tuple, datatile=None)

        assert self.result.to_string() == '   a  b\n1  2  4\n2  3  5'
        assert self.patch_update == False

    @pytest.mark.parametrize('new_indices, result', [([4,5], '   a  b\n1  2  4\n2  3  5'), 
                            ([], '   a  b\n0  1  3\n1  2  4\n2  3  5\n3  4  6'), ([3], '   a  b\n0  1  3')])
    def test_query_chart_by_indices(self, new_indices, result):
        ...
        #creating a dummy reload chart fn as its not implemented in core non aggregate chart class
        bsl.reload_chart = t_func
        bsl.query_chart_by_indices(active_chart = bsl_1, old_indices=[], new_indices=new_indices, datatile=None)
