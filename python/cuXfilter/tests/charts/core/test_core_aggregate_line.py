...
class TestBaseLine:
    ...
    def test_variables(self):
        ...
        assert bl.reset_event == None
        assert bl._datatile_loaded_state == False
        assert bl.filter_widget == None
        assert bl.use_data_tiles == True
        assert bl.x == 'test_x'
        assert bl.y == None
        assert bl.data_points == int(100)
        assert bl.add_interaction == True
        assert bl.aggregate_fn == 'count'
        assert bl.width == 400
        assert bl.height == 400
        assert bl.stride == None
    ...
    @pytest.mark.parametrize('bl, result',[
        ...
        (BaseLine(x='key'), {'X': [0.0, 0.8, 1.6, 2.4000000000000004, 4.0], 'Y': [1, 1, 1, 1, 1]})
    ])
    ...
    @pytest.mark.parametrize('range, query',[
        ((3,4), '3<=key<=4'),
         ((0,0),'0<=key<=0')
         ])
