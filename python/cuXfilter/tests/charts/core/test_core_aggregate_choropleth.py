...
from cuXfilter.charts.core.aggregate.core_aggregate_choropleth import BaseChoropleth
import cuXfilter
...
class TestBaseCholorpleth:
    ...
    def test_variables(self):
        bc = BaseChoropleth(x='key', geoJSONSource=self.geoJSONSource, nan_color='white')
        result = geo_json_mapper(self.geoJSONSource, None)
        
        assert bc.chart_type == 'choropleth'
        assert bc.reset_event == None
        assert bc._datatile_loaded_state == False
        assert bc.geo_mapper == result[0]
        assert bc.use_data_tiles == True
        assert bc.x == 'key'
        assert bc.y == None
        ...
        assert bc.stride == None
        assert bc.stride_type == int
        assert bc.geoJSONSource == self.geoJSONSource
        assert bc.geoJSONProperty == None
        assert bc.geo_color_palette == None
        assert bc.tile_provider == None
    ...
    def test_get_selection_callback(self, old, new):
        bc = BaseChoropleth(x='key',y='val', geoJSONSource=self.geoJSONSource, nan_color='white')
        dashboard = self.cux_df.dashboard(charts=[bc], title='test_title')
        self.result = None
        def func1(old, new):
            self.result = (old, new)
    ...
    @pytest.mark.parametrize('event, add_interaction, result1, result2',[
