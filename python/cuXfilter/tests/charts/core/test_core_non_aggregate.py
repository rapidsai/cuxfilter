...
from cuXfilter.charts.core.non_aggregate.core_non_aggregate import BaseNonAggregate
...
class TestCoreAggregateChart():
    ...
    def test_variables(self):
        ...
        #BaseChart variables
        assert bnac.chart_type == None
        assert bnac.x == None
        assert bnac.y == None
        assert bnac.aggregate_fn == 'count'
        assert bnac.color == None
        assert bnac.height == 0
        assert bnac.width == 0
        assert bnac.add_interaction == True
        assert bnac.chart == None
        assert bnac.source == None
        assert bnac.source_backup == None
        assert bnac.data_points == 0
        assert bnac._library_specific_params == {}
        assert bnac._stride == None
        ...
        assert bnac.use_data_tiles == False
        assert bnac.reset_event == None
        assert bnac.x_range == None
        assert bnac.y_range == None
        assert bnac.aggregate_col == None
    ...
    def test_get_selection_geometry_callback(self):
        ...
        assert bnac.get_selection_geometry_callback(dashboard).__name__ == 'selection_callback'
        assert type(bnac.get_selection_geometry_callback(dashboard)) == types.FunctionType
    ...
    def test_query_chart_by_range(self):
        ...
        bnac.query_chart_by_range(active_chart = bnac_1, query_tuple=query_tuple, datatile=None)
    ...
    def test_query_chart_by_indices(self, new_indices, result):
        ...
        bnac.query_chart_by_indices(active_chart = bnac_1, old_indices=[], new_indices=new_indices, datatile=None)
