import pytest

import cuXfilter
from cuXfilter.charts.core.core_widget import BaseWidget

class TestBaseWidget():

    def test_variables(self):
        bw = BaseWidget('test_x', value=1, label_map={'temp':1})

        assert bw.chart_type == None
        assert bw.x == 'test_x'
        assert bw.color == None
        assert bw.height == 10
        assert bw.width == 400
        assert bw.chart == None
        assert bw.data_points == 100
        assert bw.start  == None
        assert bw.end == None
        assert bw._stride == None
        assert bw.stride_type == int
        assert bw.min_value == 0.0
        assert bw.max_value == 0.0
        assert bw.use_data_tiles == False
        assert bw.value == 1
        assert bw.label_map == {1: 'temp'}
        assert bw.params == {}

        assert bw.compute_query_dict({}) == None
        assert bw.reload_chart() == -1
    
    @pytest.mark.parametrize('chart, _chart',[(None, None),(1,1)])
    def test_view(self, chart, _chart):
        bw = BaseWidget('test_x')
        bw.chart = chart
        
        assert bw.view() == _chart
    
    def test_add_event(self):
        bw = BaseWidget('test_x')

        from bokeh.events import ButtonClick
        from bokeh.models import Button

        def callback(event):
            print('Python:Click')

        bw.chart = Button()
        bw.add_event(ButtonClick,callback)

        assert ButtonClick.event_name in bw.chart.subscribed_events
