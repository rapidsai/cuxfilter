...
class TestBaseScatterGeo:
    def test_variables(self):
        bsg = BaseScatterGeo(
            x="test_x", y="test_y", aggregate_col="test_agg_col"
        )

        assert bsg.chart_type == "scatter-geo"
        assert bsg.x == "test_x"
        assert bsg.y == "test_y"
        assert bsg.reset_event is None
        assert bsg.x_range is None
        assert bsg.y_range is None
        assert bsg.add_interaction is True
        assert bsg.color_palette is None
