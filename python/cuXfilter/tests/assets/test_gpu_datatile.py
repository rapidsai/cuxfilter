...
def test_calc_cumsum_data_tile():
    ...
    groupby_as_ndarray = cuda.to_device(
        ...
        ).agg({'val': 'count'}).to_pandas().values.astype(float)
    )
...
def test_format_result(result, return_format_str, return_format):
    assert type(
        gpu_datatile.format_result(result, return_format_str)
    ) == return_format
...
def test_calc_data_tile_for_size():
    ...
    return_result = gpu_datatile.calc_data_tile_for_size(
        df=df, col_1=col_1, min_1=min_1, max_1=max_1,
        stride_1=stride_1, cumsum=cumsum)
...
def test_calc_data_tile():
    ...
    return_result = gpu_datatile.calc_data_tile(
        df=df, active_view=active_chart,
        passive_view=passive_chart, cumsum=cumsum
    )
