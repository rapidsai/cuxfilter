...
class BaseStackedLine(BaseChart):
    '''
        ...
        If dataset size is greater than a few thousand points, non aggreagated charts can crash the browser tabs,
        and is only recommended with cudatashader plugin, in which case an image is rendered instead of points on canvas
    '''
    ...
    def __init__(self, x, y=[], data_points=100, add_interaction=True,  colors=[], step_size=None, step_size_type=int, width=800, height=400,  **library_specific_params):
        '''
        Description:
    ...
    def initiate_chart(self, dashboard_cls):
        ...
        if self.y_range is None:
            ...
            self.y_range = (dashboard_cls._data[self.y].min().min(), dashboard_cls._data[self.y].max().max())
    ...
    def get_selection_geometry_callback(self, dashboard_cls):
        ...
        def selection_callback(xmin, xmax, ymin, ymax):
            ...
            dashboard_cls._query_str_dict[self.name] = str(xmin)+"<="+self.x+" <= "+str(xmax)
    ...
    def compute_query_dict(self, query_str_dict):
        ...
        if self.x_range is not None and self.y_range is not None:
            query_str_dict[self.name] = str(self.x_range[0])+"<="+self.x+" <= "+str(self.x_range[1])
